...
const parser = new ArgumentParser({
  addHelp: true,
  description:
    "Whether to show bundle content as convenient interactive zoomable treemap",
});
...
compiler.run(err => {
  if (err) console.log(err);
    ...
    setTimeout(() => {
      shell.mv(
        path.join(buildFolder, "./static/main.js"),
        path.join(buildFolder, `/static/main.${timestamp}.js`),
      );
