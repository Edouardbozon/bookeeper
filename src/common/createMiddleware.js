...
export default function createMiddleware(handlers) {
  return store => next => action => {
    ...
    const actionHandler = head(
      filter(
        handler =>
          any(handlerAction => handlerAction === action.type)(handler.actions),
        handlers,
      ),
    );

    // custom behavior before the action is dispatched
    ...
    // custom behavior after the action is dispatched
